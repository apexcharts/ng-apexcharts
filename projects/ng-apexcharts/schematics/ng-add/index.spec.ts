...
describe("ng add ng-apexcharts", () => {
  ...
  it("should add apexcharts to package.json", async () => {
    ...
    let packageJson = tree.readJson("package.json") as {
      dependencies: { apexcharts: string };
    };
  ...
  it("add NgApexchartsModule in angular no standalone project", async () => {
    ...
    expect(
      angularJson.projects[options.project].architect.build.options.scripts,
    ).toEqual(
    ...
    expect(
      angularJson.projects[options.project].architect.test.options.scripts,
    ).toEqual(
      jasmine.arrayContaining([
        "/node_modules/apexcharts/dist/apexcharts.min.js",
      ]),
